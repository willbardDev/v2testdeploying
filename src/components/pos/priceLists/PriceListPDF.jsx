...
function PriceListPDF({priceList,authObject : {authUser : {user}, authOrganization: { organization }}}) { 
    const mainColor = organization.settings?.main_color || "#2113AD";
    const lightColor = organization.settings?.light_color || "#bec5da";
    const contrastText = organization.settings?.contrast_text || "#FFFFFF";
    const is_vat_registered = organization.settings?.vat_registered;
    const vat_percentage = organization.settings?.vat_percentage;
                    ...
                    priceList.items.map((priceItem,index) => (
                        <View key={priceItem.id} style={pdfStyles.tableRow}>
                            ...
                            <Text style={{ ...pdfStyles.tableCell, backgroundColor: index % 2 === 0 ? '#FFFFFF' : lightColor, flex : 1.2, textAlign : 'right'  }}>{(priceItem.price).toLocaleString()}</Text>
                            {
                                is_vat_registered &&
                                <>
                                    <Text style={{ ...pdfStyles.tableCell,backgroundColor: index % 2 === 0 ? '#FFFFFF' : lightColor, flex : 1.2, textAlign : 'right'}}>{(!priceItem.product?.vat_exempted ? vat_percentage*priceItem.price*0.01 : 0).toLocaleString('en-US',{maximumFractionDigits:2,minimumFractionDigits:2})}</Text> 
                                    <Text style={{ ...pdfStyles.tableCell, backgroundColor: index % 2 === 0 ? '#FFFFFF' : lightColor, flex : 1.2, textAlign : 'right' }}>{(priceItem.price*(!priceItem.product?.vat_exempted ? (100+vat_percentage)*0.01 : 1)).toLocaleString('en-US',{maximumFractionDigits:2,minimumFractionDigits:2})}</Text>
                                </>
                            }
