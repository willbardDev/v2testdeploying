...
function ProformaSaleForm({proforma=null, toggleOpen}) {
    const {enqueueSnackbar} =  useSnackbar();
    const queryClient = useQueryClient();
    const {activeOutlet} = useSalesOutlet();
    const { stores, cost_center } = activeOutlet;
    ...
    const addSale = useMutation(posServices.addSale,{
        onSuccess: (data) => {
            toggleOpen(false);
    ...
    useEffect(() => {
        if(activeOutlet.counters[0]?.id){
            setCounterLedgers(activeOutlet.counters[0].ledgers);
            setValue('sales_outlet_counter_id',activeOutlet.counters[0].id);
        }
    }, [activeOutlet.counters[0]]);
    ...
    const {setValue, register, handleSubmit, watch, clearErrors, formState : {errors}} = useForm({
        ...
        defaultValues: {
            ...
            sales_outlet_counter_id: activeOutlet.counters[0]?.id,
    ...
    const {data: stakeholderReceivableLedgers,isLoading : isLoadingReceivableLedgers} = useQuery(['stakeholderReceivableLedgers',{ stakeholderId : watch('stakeholder_id')}], async() => {
    ...
    //setvalue of instant_sale true, on form opening
    useEffect(() => {
                ...
                <Grid item xs={12} md={9} mb={2}>
                    <form autoComplete='off'>
                            ...
                            </Grid>
                        {
                            ...
                            <Grid item md={watch('currency_id') > 1 ? 6 : 4} lg={watch('currency_id') > 1 ? 12 : 4} xs={12} >
                                <Div sx={{mt: 0.3}}>
                                    <StakeholderSelector
                                        label='Client'
                    ...
                    {items.map((item, index) => (
                        <React.Fragment key={item.id}>
                            ...
                            >
                                ...
                                <Grid item xs={0.7} md={0.5}>
                                    <Div sx={{ mt: 1.7, mb: 1.7 }}>{index + 1}.</Div>
                                </Grid>
                                <Grid item xs={9.3} md={6} lg={4}>
                                ...
                                <Grid textAlign={'center'} item xs={2} md={5.5} lg={1}>
                                    <Div sx={{ mt: 1.7, mb: 1.7 }}>
                                ...
                                <Grid item xs={6} md={6} lg={checkedForInstantSale ? (watch(`items.${index}.store_id`) ? 1.5 : 2.2) : 3.25}>
                                    <Div sx={{ mt: 0.7, mb: 0.5 }}>
                                        <TextField
                                            label='Quantity'
                                            fullWidth
