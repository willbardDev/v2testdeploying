...
function SalePDF({sale,organization,thermalPrinter}) {
        ...
        <Page size={[80 * 2.83465, 297 * 2.83465]} style={{...pdfStyles.page, padding: 10, scale: 0.7 }}>
            ...
            <View style={{ ...pdfStyles.tableRow, marginBottom: 10 }}>
                <View style={{ flex: 1, padding: 1 }}>
                    ...
                    <Text style={{...pdfStyles.midInfo, fontWeight: 'bold', textAlign: 'center'}}>{sale.stakeholder.name}</Text>
                ...
                </View>
                {
                    sale.sale_items.map((saleItem,index) => (
                        <View key={saleItem.id} style={{ ...pdfStyles.tableRow,borderTop: '1px', borderTopStyle: 'solid', borderTopColor: mainColor }}>
                            ...
                            <Text style={{ ...pdfStyles.tableCell, backgroundColor: index % 2 === 0 ? '#FFFFFF' : lightColor, flex : 3}}>{saleItem.product.name}</Text>
                            <Text style={{ ...pdfStyles.tableCell, backgroundColor: index % 2 === 0 ? '#FFFFFF' : lightColor, flex : 1.3,textAlign : 'right'  }}>{`${saleItem.quantity} ${(saleItem.measurement_unit.symbol)}`}</Text>
                            <Text style={{ ...pdfStyles.tableCell, backgroundColor: index % 2 === 0 ? '#FFFFFF' : lightColor, flex : 1.5,textAlign : 'right'  }}>{(saleItem.rate*(1+(saleItem?.vat_exempted !== 1 ? sale.vat_percentage*0.01 : 0))).toLocaleString().toLocaleString('en-US',{maximumFractionDigits:2,minimumFractionDigits:2})}</Text>
                            <Text style={{ ...pdfStyles.tableCell, backgroundColor: index % 2 === 0 ? '#FFFFFF' : lightColor, flex : 1.7,textAlign : 'right'  }}>{(saleItem.quantity*saleItem.rate*(1+(saleItem?.vat_exempted !== 1 ? sale.vat_percentage*0.01 : 0))).toLocaleString('en-US',{maximumFractionDigits:2})}</Text>
                        </View>
                    ))
                }
            </View> 
            <View style={{...pdfStyles.tableRow, marginTop: 5}}>
                ...
                <Text style={{ ...pdfStyles.tableHeader, ...pdfStyles.tableCell, backgroundColor: mainColor, color: contrastText, flex : 2.2,textAlign : 'right'}}>{sale.amount.toLocaleString("en-US", {style:"currency", currency:currencyCode})}</Text>
            </View>
            {
                ...
                <React.Fragment>
                    <View style={{ ...pdfStyles.tableRow, marginTop: 2 }}>
                        ...
                        <Text style={{...pdfStyles.tableCell, backgroundColor: mainColor, color: contrastText, flex : 2.2,textAlign : 'right'}}>{sale.vat_amount.toLocaleString("en-US", {style:"currency", currency:currencyCode})}</Text>
                    </View>
                    <View style={{ ...pdfStyles.tableRow, marginTop: 2 }}>
                        <Text style={{...pdfStyles.tableCell, backgroundColor: mainColor, color: contrastText, flex : 2}}>Grand Total (VAT Incl.)</Text>
                        <Text style={{...pdfStyles.tableCell, backgroundColor: mainColor, color: contrastText, flex : 2.2,textAlign : 'right'}}>{(sale.amount + sale.vat_amount).toLocaleString("en-US", {style:"currency", currency:currencyCode})}</Text>
                    </View>
                </React.Fragment>
            }
    ...
    const PDFA4 = () => (
        <Page size="A4" style={pdfStyles.page}>
            ...
            <DocumentStakeholders fromLabel={'SUPPLIER'} toLabel={'CLIENT'} stakeholder={sale.stakeholder} organization={organization}/>
            <View style={{ ...pdfStyles.tableRow, marginBottom:10}}>
                <View style={{ flex: 1, padding: 2}}>
                    <Text style={{...pdfStyles.minInfo, color: mainColor }}>Sale Date & Time</Text>
                    <Text style={{...pdfStyles.minInfo }}>{readableDate(sale.transaction_date, true)}</Text>
            ...
            <View style={{...pdfStyles.table, minHeight: 230 }}>
                <View style={pdfStyles.tableRow}>
                    ...
                    <Text style={{ ...pdfStyles.tableCell, ...pdfStyles.tableHeader, ...pdfStyles.midInfo, backgroundColor: mainColor, color: contrastText, flex : 1.2 }}>Price { sale?.vat_percentage ? ' (Excl. )' : ''}</Text>
                            {
                                sale?.vat_percentage > 0 &&
                                <Text style={{ ...pdfStyles.tableCell, ...pdfStyles.tableHeader, ...pdfStyles.midInfo, backgroundColor: mainColor, color: contrastText, flex : 1 }}>VAT</Text>
                            }
                            <Text style={{ ...pdfStyles.tableCell, ...pdfStyles.tableHeader, ...pdfStyles.midInfo, backgroundColor: mainColor, color: contrastText, flex : 1.5 }}>Amount { sale?.vat_percentage ? ' (Incl. )' : ''}</Text>
                </View>
                {
                    sale.sale_items.map((saleItem,index) => (
                        <View key={saleItem.id} style={pdfStyles.tableRow}>
                            ...
                            <Text style={{ ...pdfStyles.tableCell, backgroundColor: index % 2 === 0 ? '#FFFFFF' : lightColor, flex : 0.9, textAlign : 'right'}}>{saleItem.quantity}</Text>
                            <Text style={{ ...pdfStyles.tableCell,backgroundColor: index % 2 === 0 ? '#FFFFFF' : lightColor, flex : 1.2, textAlign : 'right'}}>{saleItem.rate.toLocaleString('en-US',{maximumFractionDigits:2,minimumFractionDigits:2})}</Text>
                                    {
                                        sale?.vat_percentage > 0 &&
                                        <Text style={{ ...pdfStyles.tableCell,backgroundColor: index % 2 === 0 ? '#FFFFFF' : lightColor, flex : 1, textAlign : 'right'}}>{(!saleItem.product?.vat_exempted ? sale.vat_percentage*saleItem.rate*0.01 : 0).toLocaleString('en-US',{maximumFractionDigits:2,minimumFractionDigits:2})}</Text> 
                                    }
                                    <Text style={{ ...pdfStyles.tableCell,backgroundColor: index % 2 === 0 ? '#FFFFFF' : lightColor, flex : 1.5, textAlign : 'right'}}>{(saleItem.quantity*saleItem.rate*(!saleItem.product?.vat_exempted ? (100+sale.vat_percentage)*0.01 : 1)).toLocaleString('en-US',{maximumFractionDigits:2,minimumFractionDigits:2})}</Text> 
                        </View>
                    ))
                }
            </View> 
            <View style={pdfStyles.tableRow}>
                ...
                <Text style={{ ...pdfStyles.tableCell, ...pdfStyles.tableHeader,backgroundColor: mainColor, color: contrastText, flex : 2.2, textAlign : 'right'   }}>{sale.amount.toLocaleString("en-US", {style:"currency", currency:currencyCode})}</Text>
            </View>
            {
                sale.vat_percentage > 0 &&
                <React.Fragment>
                    <View style={{ ...pdfStyles.tableRow, marginTop: 4 }}>
                        ...
                        <Text style={{...pdfStyles.tableHeader, backgroundColor: mainColor, color: contrastText, flex : 2.2, textAlign : 'right'   }}>{sale.vat_amount.toLocaleString("en-US", {style:"currency", currency:currencyCode})}</Text>
                    </View>
                    <View style={{ ...pdfStyles.tableRow, marginTop: 4 }}>
                        <Text style={{ textAlign: 'center', flex : 4.5  }}></Text>
                        ...
                        <Text style={{...pdfStyles.tableHeader, backgroundColor: mainColor, color: contrastText, flex : 2.2, textAlign : 'right'   }}>{(sale.amount + sale.vat_amount).toLocaleString("en-US", {style:"currency", currency:currencyCode})}</Text>
                    </View>
                </React.Fragment>
            }
