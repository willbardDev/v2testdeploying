...
import { Alert, Box, Chip, Grid, IconButton, LinearProgress, Tooltip, Typography, useMediaQuery } from '@mui/material';
import { readableDate } from 'app/helpers/input-sanitization-helpers';
import { AttachmentOutlined, ContentPasteSearchOutlined, DeleteOutlined, EditOutlined, LocalShipping } from '@mui/icons-material';
...
function SaleDeliveryNotes({expanded, sale}) {
  ...
  return (
    <React.Fragment>
        ...
        {
            saleDeliveryNotes?.length > 0 ? saleDeliveryNotes.map((deliveryNote) => (
                <Grid
                    key={deliveryNote.id}
                    sx={{
                        cursor: 'pointer',
                ...
                <Grid item xs={6} md={2} lg={3} mt={0.5}>
                    {
                ...
                <Grid item xs={12} md={3} lg={2}>
                    ...
                    >
                        ...
                        {(checkOrganizationPermission(PERMISSIONS.SALES_DISPATCH_BACKDATE) || deliveryNote.dispatch_date >= dayjs().startOf('date').toISOString()) && !deliveryNote.is_invoiced &&
            ...
            :
            !isLoading && <Alert variant='outlined' color='primary' severity='info'>No dispatch found</Alert>       
        }
