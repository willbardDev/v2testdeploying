import { Accordion, AccordionDetails, AccordionSummary, Chip, Divider, Grid, Tooltip, Typography } from '@mui/material'
...
function CounterSalesListItem({sale}) {
  ...
  return (
    ...
    >
      ...
      <Grid item xs={12} md={4} lg={3} display={'flex'} alignItems={'center'} justifyContent={'space-between'}>
        <Tooltip title='Status'>
          <Chip
            size='small' 
            label={sale.status}
            color={(sale.status === 'Complete'|| sale.status === 'Fulfilled') ? 'success' : sale.status === 'Partially Fulfilled' ? 'info' : sale.status === 'Over Fulfilled' ? 'warning' : 'primary'}
          />
        </Tooltip>
        {
          sale.is_fully_paid && 
        ...
        <Tooltip title='Amount'>
          <Typography>{(sale.amount + sale.vat_amount).toLocaleString("en-US", {style:"currency", currency:sale.currency.code})}</Typography>
        </Tooltip>
      </Grid>
    </Grid>
        <Divider/>
      ...
      >
        <Grid container>
          <Grid item xs={12} textAlign={'end'}>
            <CounterSalesListItemAction sale={sale} openDispatchDialog={openDispatchDialog} setOpenDispatchDialog={setOpenDispatchDialog}/>
          </Grid>

          {/*Tabs*/}
          {
            (!sale.is_instant_sale || !!sale.is_invoiceable || !!sale.is_invoiced) &&
            <SalesListItemTabs sale={sale} activeTab={activeTab} expanded={expanded} setActiveTab={setActiveTab}/>
          }
        </Grid>
      </AccordionDetails>
    </Accordion>
...
export default CounterSalesListItem