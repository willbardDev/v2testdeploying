...
function SalesDispatchReport({organization,dispatchReport}) {
  ...
  return (
    ...
    >
        <Page size={dispatchedItems.length > 8 ? "A3" : "A4"} orientation={dispatchedItems.length > 3 ? 'landscape' : null} style={pdfStyles.page}>
            ...
            <View style={{ ...pdfStyles.table, minHeight: 230 }}>
                <View style={pdfStyles.tableRow}>
                    ...
                    {dispatchedItems.map(item => (
                        <View key={item.deliveryNo} style={{ ...pdfStyles.tableCell, ...pdfStyles.tableHeader, backgroundColor: mainColor, color: contrastText, flex: 1.5 }}>
                    ...
                    return (
                        <View key={item.id} style={pdfStyles.tableRow}>
                            ...
                            {dispatchedItems.map((dispatchedItem) => {
                                const product = dispatchedItem.products.find(p => p.productId === item.id);
                                return (
                                    <Text key={dispatchedItem.deliveryNo} style={{ ...pdfStyles.tableCell, backgroundColor: index % 2 === 0 ? '#FFFFFF' : lightColor, flex: 1.5, textAlign: 'right' }}>
