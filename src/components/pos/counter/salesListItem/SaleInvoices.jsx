...
function SaleInvoices({expanded, sale, activeTab}) {
  ...
  const { data: saleInvoices, isLoading } = useQuery(
    ['SaleInvoices', {saleId: sale.id}],() => posServices.saleInvoices(sale.id),
    {
      enabled: !!expanded && activeTab === (!sale.is_instant_sale ? 2 : (sale.payment_method === 'On Account' ? 1 : 0)),
    }
  );
  ...
  return (
    <>
      ...
      {
        saleInvoices?.length > 0 ? saleInvoices.map((invoice) => (
          <Grid
          key={invoice.id}
            sx={{
              cursor: 'pointer',
        ...
        >
          ...
          <Grid item xs={12} md={4} lg={4} mt={0.5}>
          ...
          <Grid item xs={12} md={12} lg={2}>
            ...
            >
                <Tooltip title={belowLargeScreen ? `Download Invoice ${invoice.invoiceNo}` : `View Invoice ${invoice.invoiceNo}`}>
                    <IconButton
                        onClick={() => {
        ...
        :
        !isLoading && <Alert variant='outlined' color='primary' severity='info'>No Invoices found</Alert> 
      }
