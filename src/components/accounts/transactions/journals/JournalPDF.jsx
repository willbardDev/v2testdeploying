...
function JournalPDF({transaction,authObject}) {
        ...
        <Page size="A4" style={pdfStyles.page}>
            <View style={{ ...pdfStyles.tableRow, marginBottom: 20 }}>
                ...
                <View style={{ flex: 1, textAlign: 'right' }}>
                    ...
                    <Text style={{ ...pdfStyles.midInfo }}>{transaction.voucherNo}</Text>{transaction.reference && <Text style={{ ...pdfStyles.minInfo }}>Ref: {transaction.reference}</Text>}
                </View>
            </View>
            ...
            <View style={{...pdfStyles.table, minHeight: 230, marginTop: 10}}>
                <View style={pdfStyles.tableRow}>
                    ...
                    <Text style={{...pdfStyles.tableHeader, backgroundColor: mainColor, color: contrastText, flex : 2 }}>Credit Account</Text>
                    <Text style={{...pdfStyles.tableHeader, backgroundColor: mainColor, color: contrastText, flex : 2 }}>Debit Account</Text>
                ...
                </View>
                {
                    transaction.items.map((transactionItem,index) => (
                        <View key={index} style={pdfStyles.tableRow}>
                            ...
                            <Text style={{ ...pdfStyles.tableCell, backgroundColor: index % 2 === 0 ? '#FFFFFF' : lightColor, flex : 2}}>{transactionItem.creditLedgerName}</Text>
