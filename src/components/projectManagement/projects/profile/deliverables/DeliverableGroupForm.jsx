...
const DeliverableGroupForm = ({ setOpenDialog, deliverableGroup=null, parentGroup=null}) => {
  ...
  const { mutate: addDeliverableGroup, isLoading } = useMutation(projectsServices.addDeliverableGroup, {
    onSuccess: (data) => {
      setOpenDialog(false);
  ...
  const { mutate: updateDeliverableGroup, isLoading : updateLoading } = useMutation(projectsServices.updateDeliverableGroup, {
    onSuccess: (data) => {
      setOpenDialog(false);
      enqueueSnackbar(data.message, { variant: 'success' });
      queryClient.invalidateQueries(['projectDeliverableGroups']);
  ...
  const saveMutation = React.useMemo(() => {
    return deliverableGroup ? updateDeliverableGroup : addDeliverableGroup;
  }, [addDeliverableGroup, updateDeliverableGroup, deliverableGroup]);
  ...
  const sameLevelGroups = deliverableGroups.filter(group => 
    group.parent_id === (parentGroup ? parentGroup.id : deliverableGroup ? deliverableGroup?.parent_group_id : null)
  )
  ...
  const validationSchema = yup.object({
    ...
    .test('check-total', function (value) {
      ...
      const totalWeightPercentages = sameLevelGroups.reduce(
          (total, grp) => total + (deliverableGroup && grp.position_index === deliverableGroup.position_index ? 0 : grp.weighted_percentage),
          0
      );
  ...
  const {
    ...
    defaultValues: {
      ...
      name: deliverableGroup && deliverableGroup.name,
      code: deliverableGroup && deliverableGroup.code,
      description: deliverableGroup && deliverableGroup.description,
      weighted_percentage: deliverableGroup && deliverableGroup.weighted_percentage,
  ...
  return (
    <>
      <DialogTitle textAlign={'center'}>{deliverableGroup ? `Edit: ${deliverableGroup.name}` : `New Deliverable Groups`}</DialogTitle>
      <DialogContent>
        <form autoComplete="off">
          <Grid container columnSpacing={1}>
            ...
            <Grid item xs={12} md={4}>
              <Div sx={{ mt: 1}}>
                <Autocomplete
                  options={deliverableGroup ? 
                    positionIndexOptions.filter(group => group.position_index !== deliverableGroup?.position_index) : // for edit group
