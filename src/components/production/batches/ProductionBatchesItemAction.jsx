...
const EditBatch = ({batch, setOpenEditDialog}) => {
  ...
  const {data:batchData,isFetching} = useQuery(['showBatchDetails',{id:batch.id}], () => productionBatchesServices.showBatchDetails(batch.id));
  ...
  return (
    <ProductionBatchesForm production={batchData} toggleOpen={setOpenEditDialog} setIsConsumptionDeletedInside={setIsConsumptionDeletedInside}/>
  )
}
  ...
  return (
    <DialogContent>
      ...
      {belowLargeScreen && activeTab === 0 ?
        ...
        :
        <PDFContent fileName={batchData.batchNo} document={<BatchPDF batch={batchData} organization={organization}/>}/>
      }
...
const ProductionBatchesItemAction = ({ batch }) => {
  ...
  const { mutate: deleteProductionBatch } = useMutation(productionBatchesServices.deleteProductionBatch, {
    onSuccess: (data) => {
  ...
  return (
    <>
      ...
      >
        ...
        {openDocumentDialog && <DocumentDialog batch={batch} organization={organization} setOpenDocumentDialog={setOpenDocumentDialog}/>}
      </Dialog>
      <JumboDdMenu
        icon={
