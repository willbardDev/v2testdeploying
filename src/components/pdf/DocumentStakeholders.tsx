...
function DocumentStakeholders({organization,stakeholder,fromLabel = 'BILL FROM',toLabel = 'BILL TO', order}) {
    ...
    <View style={{ ...pdfStyles.tableRow, marginBottom: 10 }}>
        ...
        <View style={{ flex: 2, padding: 1, textAlign: 'right' }}>
            ...
            <View style={{ ...pdfStyles.tableRow, ...pdfStyles.midInfo, marginTop: 5 }}>
                <View style={{ textAlign: 'right', flex: 1}}><Text style={{ color: mainColor}}>{`Paid From: `}</Text></View>
                <View style={{ ...pdfStyles.minInfo,textAlign: 'right', flex: 1}}><Text>{order.paid_from.name}</Text></View>
            </View>}
        </View>
    </View> 
  )
