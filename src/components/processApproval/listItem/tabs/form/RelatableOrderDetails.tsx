...
import { Typography, Table, TableBody, TableCell, TableContainer, TableHead, TableRow, Paper, Divider, DialogActions, Button, DialogContent, Grid } from '@mui/material';
import { useJumboAuth } from '@/app/providers/JumboAuthProvider';
import { PERMISSIONS } from '@/utilities/constants/permissions';
import { readableDate } from '@/app/helpers/input-sanitization-helpers';

function RelatableOrderDetails({ order, toggleOpen }) {
                                ...
                                <TableHead>
                                    <TableRow>
                                        ...
                                        <TableCell sx={{backgroundColor: mainColor, color: contrastText, textAlign: 'right'}}>Quantity</TableCell>
                                        {
                                            withPrices &&
                                            <>
                                                <TableCell sx={{backgroundColor: mainColor, color: contrastText, textAlign: 'right'}}>Price { vatAmount > 0 ? ' (Excl.)' : ''}</TableCell>
                                                {vatAmount > 0 &&
                                                    <TableCell sx={{backgroundColor: mainColor, color: contrastText, textAlign: 'right'}}>VAT</TableCell>
                                                }
                                                <TableCell sx={{backgroundColor: mainColor, color: contrastText, textAlign: 'right'}}>Amount { vatAmount > 0 ? ' (Incl.)' : ''}</TableCell>
                                            </>
                                        }
                                    </TableRow>
                                </TableHead>
                                            ...
                                            <TableRow key={orderItem.id} sx={{ backgroundColor: index % 2 === 0 ? '#FFFFFF' : lightColor }}>
                                                ...
                                                <TableCell align="right">{orderItem.quantity}</TableCell>
                                                {
                                                    ...
                                                    <>
                                                        ...
                                                        {vatAmount > 0 &&
                                                            <TableCell align="right">{(orderItem.rate* orderItem.quantity * orderItem.vat_percentage*0.01).toLocaleString('en-US',{maximumFractionDigits:2,minimumFractionDigits:2})}</TableCell>
                                                        }
